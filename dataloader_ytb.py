...
class YTVOSV2(torch.utils.data.Dataset):
    ...
    def _init_data(self):
        ...
        if self._split == 'train':
            with open(os.path.join(self._root_path, self._split, "ImageSets", self._image_set + '.txt'), 'r') as f:
        ...
        query_file = '../data/Youtube-VOS/meta_expressions_{}.txt' .format(self._split)
        self._query_seqs = []
        self._query = open(query_file).read().splitlines()
        if self._split == 'train':
            for query in self._query:
                if query.split(' ')[0] in self._viable_seqs:
